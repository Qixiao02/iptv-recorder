...
### ✨ 新功能
- **计划时间设置改为小白友好的简单模式**：新建/编辑计划不再需要写 cron 表达式。改为「简单模式 / 高级模式」双模式：
  - **简单模式**（默认）：开始时间用原生时间选择器（HH:MM 下拉）、星期用 7 个可切换按钮（一~日，多选，空选=每天）、录制时长改为「小时+分钟」两个输入框（告别「秒」的心算）。操作时实时显示「生成的 Cron」预览（如 `0 19 * * 1-5`），让用户对实际规则有感知。
  - **高级模式**（折叠保留）：原 cron 输入框 + 预设按钮 + 帮助表格，供「每小时」「每 N 分钟」等高频规则使用。
  - **智能解析**：编辑现有计划时自动把 cron 解析回简单 UI（能解析就回填并留在简单模式），解析不了的高频规则自动切到高级模式，不强迫用户。新增 `lib/cronBuilder.ts` 实现 cron 与简单 UI 的双向转换。

### 🔧 工程
- **默认外部端口统一为 5133**：两个 compose（开发版原 3033、部署版原 5669）统一为 5133，与 Tan-Nas 实际部署一致，避免混淆。
